...
// IDs des arrets DE DEPART du quartier (champ "id" dans tclpassagearret)
const ARRETS = {
...
// Lignes reellement desservies par pole (filtre anti-erreur source TCL).
// Si une ligne du flux n'apparait pas ici pour un pole, le passage est ignore.
// Mettre [] sur un pole = pas de filtre applique pour ce pole (tout est accepte).
...
exports.handler = async () => {
  ...
  try {
    const auth = "Basic " + Buffer.from("demo:demo4dev").toString("base64");
    ...
    const result = await httpGet(url, apiHeaders);

    if (result.status === 401) throw new Error("Authentification refusee (401)");
    if (result.status !== 200) throw new Error("HTTP " + result.status);

    const values = result.body.values || [];
    if (values.length === 0) {
      return { statusCode: 200, headers, body: JSON.stringify({ poles: {}, alertes: [], maj: new Date().toISOString(), vide: true }) };
    }

    // Construire la liste des passages valides (filtre ID + filtre ligne/pole)
    const allPassages = [];
    for (const v of values) {
      ...
      // Filtre anti-erreur source : ligne doit etre dans la liste connue du pole
    ...
    // Trier par delai croissant
    allPassages.sort((a,b) => a.delai - b.delai);

    // Dedoublonnage renforce :
    // 1) cle stricte pole+ligne+direction normalisee (gere variantes de casse/espaces)
    // 2) cle "a quai" pole+ligne uniquement quand delai<=0, pour eviter plusieurs
    //    vehicules "a quai" simultanes sur la meme ligne meme avec direction differente
    //    (cas frequent en terminus/rebroussement)
    const poles = {};
    ...
    try {
      ...
      if (resA.status === 200 && resA.body.values?.length) {
        alertes = resA.body.values.map(a => ({
          titre: a.titre || "",
          message: a.message || "",
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ poles, alertes, maj: new Date().toISOString() }) };

  } catch(e) {
    return { statusCode: 200, headers, body: JSON.stringify({ error: e.message, poles: {}, alertes: [], maj: new Date().toISOString() }) };
  }
};
